...
class AccountMoveLine(models.Model):
    ...
    l10n_cr_edi_amount_discount = fields.Monetary(
        "Unit Discount",
        compute='_compute_l10n_cr_edi_amount_discount',
        inverse='_inverse_discount',
        store=True,
        help="Discount to be applied for each product in the invoice lines ",
    )
    l10n_cr_edi_total_discount = fields.Monetary(
        "Total Discount",
        compute='_compute_l10n_cr_edi_total_discount',
    ...
    def _compute_l10n_cr_edi_amount_discount(self):
        ...
        for line in self:
            ...
            line.l10n_cr_edi_amount_discount = discount

    @api.onchange('l10n_cr_edi_amount_discount')
    def onchange_l10n_cr_edi_amount_discount(self):
        deci_pre = self.env['decimal.precision']
        discount_digits = deci_pre.precision_get('Discount')
        for line in self:
            if not line.price_unit or not line.quantity:
                continue
            total_discount = (line.l10n_cr_edi_amount_discount *
                              float_round(
                                  line.quantity,
                                  precision_digits=discount_digits))
            discount = float_round(
                float_round(line.l10n_cr_edi_total_discount,
                            precision_digits=discount_digits) /
                float_round(line.quantity,
                            precision_digits=discount_digits),
                precision_digits=discount_digits
            )
            line.l10n_cr_edi_total_discount = total_discount
            line.discount = float_round(
                discount / line.price_unit * 100,
                precision_digits=discount_digits)

    @api.depends('l10n_cr_edi_amount_discount')
    def _compute_l10n_cr_edi_total_discount(self):
        deci_pre = self.env['decimal.precision']
        discount_digits = deci_pre.precision_get('Discount')
        for line in self:
            total_discount = line.l10n_cr_edi_amount_discount * float_round(
    ...
    def _inverse_total_discount(self):
        ...
        for line in self.filtered(lambda l: l.price_unit and l.quantity):
            discount = float_round(
                float_round(line.l10n_cr_edi_total_discount, precision_digits=discount_digits) / float_round(
                    line.quantity, precision_digits=discount_digits),
                precision_digits=discount_digits
            )
            line.l10n_cr_edi_amount_discount = discount
    ...
    def _inverse_discount(self):
        ...
        for line in self.filtered('price_unit'):
            discount = float_round(
                (float_round(line.l10n_cr_edi_amount_discount,
...
class AccountMove(models.Model):
    ...
    def _compute_discount(self):
        for inv in self:
            discount = sum(
                inv.invoice_line_ids.mapped('l10n_cr_edi_total_discount'))
            inv.l10n_cr_edi_total_discount = discount
